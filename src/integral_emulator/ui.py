...
"""Static browser dashboard for local MVP checks."""
...
<head>
  ...
  <title>INTEGRAL EMULATOR Local Console</title>
  <style>
    :root {
      color-scheme: light;
      --bg: #f7f8f3;
    ...
    * { box-sizing: border-box; }
    ...
    .status.ok { color: #17684f; border-color: #a8cbbf; }
    .status.warn { color: var(--warn); border-color: #e0c58a; }
...
<body>
  <header>
    <h1>INTEGRAL EMULATOR Local Console</h1>
    <span id="health" class="status warn">checking</span>
  </header>
  <main>
    <div class="stack">
      <section class="stack">
        <h2>Demo</h2>
        <div class="row">
          <button id="runDemo">Run full demo</button>
          <button id="runSelfDemo" class="secondary">Self 2ROM dry run</button>
          <button id="refreshButton" class="secondary">Refresh lists</button>
          <button id="healthButton" class="secondary">Health</button>
          <button id="recoverButton" class="danger">Recover orphans</button>
        </div>
        <label>Username prefix
          <input id="prefix" value="local_player">
        </label>
      </section>
      <section class="stack">
        <h2>Current Result</h2>
        <div class="metrics">
          <div class="metric"><span>Session</span><strong id="sessionStatus">-</strong></div>
          <div class="metric"><span>SAV</span><strong id="saveStatus">-</strong></div>
          <div class="metric"><span>ROM</span><strong id="romStatus">-</strong></div>
          <div class="metric"><span>Host</span><strong id="hostStatus">-</strong></div>
        </div>
      </section>
    </div>
    <section class="stack">
  ...
  <script>
    ...
    const runSelfDemoButton = document.querySelector("#runSelfDemo");
    ...
    async function checkHealth() {
      ...
      healthEl.textContent = result.ok ? "online" : "offline";
      healthEl.className = result.ok ? "status ok" : "status warn";
      write("health", result);
    }
    ...
    async function runFullDemo() {
      ...
      try {
        write("demo disabled", "Direct link session creation is closed. Use ROOM matching to start link play.");
      }
      catch (error) {
    ...
    async function runSelfDemo() {
      runSelfDemoButton.disabled = true;
      try {
        write("self demo disabled", "Direct self link session creation is closed. Use ROOM matching to start link play.");
      }
      catch (error) {
        write("error", error.message);
      }
      finally {
        runSelfDemoButton.disabled = false;
      }
    }

    async function refreshLists() {
      if (!lastToken) {
        write("refresh", "Run a demo first.");
        return;
      }
      const saves = await api("GET", "/saves", undefined, lastToken);
      const sessions = await api("GET", "/link-sessions", undefined, lastToken);
      write("saves", saves);
      write("link sessions", sessions);
      if (lastSessionId) {
        const events = await api("GET", `/link-sessions/${lastSessionId}/events`, undefined, lastToken);
        write("latest session events", events);
      }
    }
    ...
    async function recoverOrphans() {
      ...
      try {
        write("recover orphans", "Disabled: recovery is no longer exposed to participant clients.");
      }
      catch (error) {
        write("error", error.message);
      }
    ...
    runDemoButton.addEventListener("click", runFullDemo);
    runSelfDemoButton.addEventListener("click", runSelfDemo);
    refreshButton.addEventListener("click", () => refreshLists().catch(error => write("error", error.message)));
    recoverButton.addEventListener("click", recoverOrphans);
    checkHealth().catch(error => write("error", error.message));
  </script>
</body>
</html>
"""
