...
class InitialSaveImportContractTests(unittest.TestCase):
    ...
    def test_clients_gate_initial_save_ui_on_server_capability(self) -> None:
        ...
        self.assertIn("server_slot_matches_registration", registration_source)
        self.assertIn("strcmp(server_slot->filename, filename) == 0", registration_source)
        self.assertIn("!same_server_registration", registration_source)
        self.assertIn("initial_save_generated ? 1 : 0", registration_source)
        rom_slots_source = (ROOT / "src/integral_emulator/rom_slots.py").read_text(encoding="utf-8")
        self.assertNotIn("INITIAL_SAVE_BYTES = bytes(32 * 1024)", rom_slots_source)
        self.assertIn("GB/GBC ROM registration requires generated initial SAV data", rom_slots_source)
