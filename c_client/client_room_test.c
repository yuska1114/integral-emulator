...
int main(int argc, char **argv)
{
    ...
    if (argc == 2 && !strcmp(argv[1], "--focus-return")) {
        ...
        app.settings.settings_selected=3;integral_client_settings_handle_settings_key(&app,&key);
        assert(app.ui.screen==SCREEN_OPTIONS && app.settings.options_selected==0);
        assert(app.settings.options_ir_off_delay_ticks==12 && !app.settings.options_sgb_enabled);
        key.keysym.sym=SDLK_RIGHT;integral_client_settings_handle_options_key(&app,&key);
        assert(app.settings.options_ir_off_delay_ticks==13);
        app.settings.options_selected=1;key.keysym.sym=SDLK_RETURN;integral_client_settings_handle_options_key(&app,&key);
        assert(app.settings.options_sgb_enabled);
        key.keysym.sym=SDLK_ESCAPE;integral_client_settings_handle_options_key(&app,&key);
        assert(app.ui.screen==SCREEN_SETTINGS && app.settings.settings_selected==3);
        key.keysym.sym=SDLK_RETURN;integral_client_settings_handle_settings_key(&app,&key);
        assert(app.settings.options_ir_off_delay_ticks==13 && app.settings.options_sgb_enabled);
        key.keysym.sym=SDLK_ESCAPE;integral_client_settings_handle_options_key(&app,&key);
        key.keysym.sym=SDLK_RETURN;
        app.settings.settings_selected=0;integral_client_settings_handle_settings_key(&app,&key);
        ...
        assert(app.ui.screen==SCREEN_SETTINGS && app.settings.settings_selected==0);
        app.settings.settings_selected=4;key.keysym.sym=SDLK_RETURN;integral_client_settings_handle_settings_key(&app,&key);
        assert(app.ui.screen==SCREEN_MAIN_MENU);
