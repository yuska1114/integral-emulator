...
static void capture_n64_room(IntegralRoomContext *state)
{
    if (strcmp(state->n64.n64_runtime_media_role, "remote") == 0) {
...
bool handle_n64_room_util_event(IntegralRoomContext *state, const SDL_Event *event)
{
    ...
    /* Only capture is owned by the background Host parent. Leave Escape's
     * game-window confirmation with the child that received the input. */
